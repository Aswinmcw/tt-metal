...
# def test_argmax_w(input_shapes, device):
#     torch.manual_seed(0)
...
#     )
#     tt_output_tensor_on_device = tt_lib.tensor.argmax(input_tensor)
#     print("Maximum value ")
#     print(torch.max(input_data))
#     golden_tensor = torch.argmax(input_data, dim)
...
def test_argmax_w(input_shapes, device):
    ...
    tt_output_tensor_on_device = tt_lib.tensor.argmax(input_tensor, dim=3)
    # print("Maximum value ")
    # print(torch.max(input_data))
    golden_tensor = torch.argmax(input_data, dim=3)
...
def test_argmax_h(input_shapes, device):
    torch.manual_seed(0)
    input_data = torch.randn(input_shapes).bfloat16()
    input_tensor = (
        tt_lib.tensor.Tensor(input_data, tt_lib.tensor.DataType.BFLOAT16).to(tt_lib.tensor.Layout.TILE).to(device)
    )
    tt_output_tensor_on_device = tt_lib.tensor.argmax(input_tensor, dim=2)
    # print("Maximum value ")
    # print(torch.max(input_data, dim =2))
    golden_tensor = torch.argmax(input_data, dim=2)
    ...
    # # print(input_data)
    # print(pt_out_tensor, tt_out_tensor)
    # print(pt_out_tensor, tt_out_tensor[0,0])
    comp_pass, comp_out = comparison_funcs.comp_pcc(pt_out_tensor, tt_out_tensor[0, 0, 0], pcc=0.99)
    comp_all, _ = comparison_funcs.comp_allclose(pt_out_tensor, tt_out_tensor[0, 0, 0], atol=4, rtol=1e-1)
    logger.info(comp_pass)
    logger.info(comp_all)
