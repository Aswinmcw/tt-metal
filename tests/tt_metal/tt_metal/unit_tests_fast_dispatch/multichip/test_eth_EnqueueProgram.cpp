...
bool test_dummy_EnqueueProgram_with_runtime_args(Device* device, const CoreCoord &eth_core_coord) {
    ...
    auto dummy_kernel0 = CreateKernel(
        program, "tests/tt_metal/tt_metal/gtest_unit_tests/command_queue/test_kernels/runtime_args_kernel0.cpp", eth_core_coord, tt_metal::experimental::EthernetConfig{.eth_mode = tt_metal::Eth::SENDER, .noc = tt_metal::NOC::NOC_0});
    ...
    vector<uint32_t> dummy_kernel0_args_readback = llrt::read_hex_vec_from_core(device->id(), eth_noc_xy, eth_l1_mem::address_map::ERISC_L1_ARG_BASE, dummy_kernel0_args.size() * sizeof(uint32_t));
...
TEST_F(CommandQueuePCIDevicesFixture, EnqueueDummyProgramOnEthCore) {
            ASSERT_TRUE(fd_unit_tests::erisc::kernels::test_dummy_EnqueueProgram_with_runtime_args(devices_.at(0), CoreCoord(0,0)));/*
...
TEST_F(CommandQueuePCIDevicesFixture, EthKernelsNocReadNoSend) {
    ...
    for (const auto &device: devices_) {
        for (const auto& eth_core : device->get_active_ethernet_cores()) {
        ASSERT_TRUE(
            fd_unit_tests::erisc::kernels::reader_kernel_no_send(device, WORD_SIZE, src_eth_l1_byte_address, eth_core));
