    ...
    if (matmul) {
        ...
        if ((input_tensor_a.memory_config().memory_layout == TensorMemoryLayout::WIDTH_SHARDED
            ...
            if (output_mem_config.is_sharded()) {
                TT_FATAL(input_tensor_a.memory_config() == output_mem_config);
            ...
            if (input_tensor_a.memory_config().memory_layout == TensorMemoryLayout::WIDTH_SHARDED) {
                mcast_in0 = true;
                per_core_M = M;
                per_core_N = N / input_tensor_a.shard_spec().value().grid.num_cores();
    ...
    return std::visit(
        [&](const auto& program_config) -> operation::ProgramWithCallbacks {
            ...
            if constexpr (std::is_same_v<ProgramConfigType, MatmulDefaultProgramConfig>) {
                ...
                broadcast_batch &= input_tensor_a.shape()[0] * input_tensor_a.shape()[1] > 1;
