...
void split_and_form_rectangle_grids(std::vector<CoreCoord>& group, std::vector<CoreCoord>& mcast_group_first, std::vector<CoreCoord>& mcast_group_last) {
    ...
    for (int i = 0; i < remove_front; ++i) {
        mcast_group_first.push_back(group.front());
        group.erase(group.begin());
    }

    // Pop and push the back outliers
    for (int i = 0; i < remove_back; ++i) {
        mcast_group_last.push_back(group.back());
        group.pop_back();
    }
}
operation::ProgramWithCallbacks groupnorm_sharded_(
    ...
    uint32_t per_core_M = a.shard_spec().value().shard_shape[0];
    uint32_t per_core_N = a.shard_spec().value().shard_shape[1];
    ...
    auto shard_orientation = a.shard_spec().value().shard_orientation;
    ...
    std::cout << "num_cores_r "<<  num_cores_r<< std::endl;
    std::cout << "num_cores_c "<<  num_cores_c<< std::endl;
    std::cout << "num_cores_per_batch "<<  num_cores_per_batch<< std::endl;
    std::cout << "num_cores_per_group " << num_cores_per_group<< std::endl;
    std::cout << "num_batches_per_core " << num_batches_per_core << std::endl;
    std::cout << "num_groups_per_core " << num_groups_per_core << std::endl;
    // subblock
    ...
    std::cout << "block_ht "<<  block_ht<< std::endl;
    std::cout << "block_w "<<  block_w<< std::endl;
    std::cout << "block_h "<<  block_h<< std::endl;
    std::cout << "subblock_wt "<<  subblock_wt<< std::endl;
    std::cout << "num_subblocks_w "<<  num_subblocks_w<< std::endl;
    // get sharded addr
    ...
    for (int i=0; i < num_batches / num_batches_per_core; ++i) {
        ...
        for (int j=0; j < num_groups / num_groups_per_core; ++j) {
            ...
            std::cout << "index "  << core_index << std::endl;
            std::cout << "mcast_sender_core_ranges " << core_coords[core_index].x << " " << core_coords[core_index].y << std::endl;
    ...
    for (int i=0; i < num_cores_r * num_cores_c; ++i) {
        ...
        if (mcast_sender_core_ranges.find(CoreRange{core_coords[i]}) == mcast_sender_core_ranges.end()) {
            ...
            std::cout << "mcast_receiver_core_ranges " << core_coords[i].x << " " << core_coords[i].y << std::endl;
        }
    }
    ...
    std::cout << " Reader cores len " <<  mcast_sender_cores.num_cores()<< std::endl;
    ...
    for (int i=0; i < mcast_groups.size(); ++i) {
        ...
        std::cout << "rectangle_grid " << rectangle_grid << std::endl;

        for (int j=0; j < group.size(); ++j) {
            CoreCoord core = group[j];
            CoreCoord core_physical = device->worker_core_from_logical_core(core);

            gamma_tile_start_id = (gamma_tile_start_id + block_wt) % Wt;
            beta_tile_start_id = (beta_tile_start_id + block_wt) % Wt;
            ...
            if (j == 0) { // mcast sender
                ...
                if (not rectangle_grid) {
                    split_and_form_rectangle_grids(group, mcast_group_first, mcast_group_last);
                }
...
std::vector<Tensor> GroupNorm::create_output_tensors(const std::vector<Tensor> &input_tensors) const {
    ...
    if (this->program_config.inplace) {
        ...
        return {create_sharded_device_tensor(this->compute_output_shapes(input_tensors).at(0), program_config.out_data_format, Layout::TILE, input_tensor.device(), this->output_mem_config, input_tensor.shard_spec().value())};
    }
}
