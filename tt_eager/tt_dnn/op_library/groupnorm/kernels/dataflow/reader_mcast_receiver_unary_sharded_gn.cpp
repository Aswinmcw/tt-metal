...
void kernel_main() {
    ...
    // DPRINT << "Reader recever !! num_group_batch " << num_group_batch << ENDL();
    ...
    for (uint32_t i=0; i < 2; ++i) {
        ...
        // DPRINT << "Reader recever !! set sem done " << mcast_sender_noc_x << " "  << mcast_sender_noc_y<< ENDL();

        noc_semaphore_wait(reduce_sender_semaphore_addr_ptr, VALID);

        // DPRINT << "Reader recever !! mcast recv done " << num_group_batch << ENDL();

        // DPRINT << TSLICE(cb_ex_global, 0, SliceRange::h0_w0_32()) << ENDL();
    ...
    // DPRINT << "Reader recever !! DONE " <<ENDL();
}
