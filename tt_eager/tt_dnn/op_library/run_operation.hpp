...
struct RunOperationState {
    ...
    void push_composite_parent_name(const char* parent_name) { this->composite_parent_names.push_back(parent_name); }

    void pop_composite_parent_name() { this->composite_parent_names.pop_back(); }

    bool is_composite_operation() const { return not composite_parent_names.empty(); }

    const auto& get_composite_parent_names() const { return this->composite_parent_names; }
...
inline void pop_composite_parent_name() { detail::OPERATION_STATE.pop_composite_parent_name(); }

inline bool is_composite_operation() { return detail::OPERATION_STATE.is_composite_operation(); }

inline const auto& get_composite_parent_names() { return detail::OPERATION_STATE.get_composite_parent_names(); }
...
inline std::vector<Tensor> run_with_autoformat(
    ...
    return run_with_autoformat(
        operation, input_tensors, input_formatting, output_layouts, optional_input_tensors, optional_input_formatting);
}

std::tuple<const std::vector<Tensor>, const std::vector<std::optional<const Tensor>>> auto_move_tensors_to_device(
    const std::vector<Tensor>& input_tensors, const std::vector<std::optional<const Tensor>>& optional_input_tensors = {});
