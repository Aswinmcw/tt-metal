...
std::pair<std::vector<uint32_t>, std::vector<uint32_t> >   get_unpad_runtime_args_rm(const Tensor &input_tensor,
    ...
    uint32_t start_offset = output_tensor_start[0] * output_tensor_start[1] * output_tensor_start[2] * output_tensor_start[3]/input_shape[3];
...
std::pair<std::vector<uint32_t>, std::vector<uint32_t> >   get_unpad_runtime_args_tile(const Tensor &input_tensor,
    ...
    uint32_t start_offset = output_tensor_start[0] * output_tensor_start[1] * output_tensor_start[2]/TILE_HEIGHT * output_tensor_start[3]/TILE_WIDTH;
    vector<uint32_t> reader_kernel_args = {
        input_tensor.buffer()->address(),
        num_dims,
        start_offset,
