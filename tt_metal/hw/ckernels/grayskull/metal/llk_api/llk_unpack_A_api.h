...
//     std::uint32_t operand_id = get_operand_id(operand);
//     std::uint32_t base_address = cb_interface[operand_id].fifo_rd_ptr - 1;
//     std::uint32_t offset_address = MUL_TILE_SIZE_AND_INDEX((uint)unpack_src_format[operand_id], 1);
...
ALWI void llk_unpack_A_cm(const std::uint32_t operand, const std::uint32_t start_tile_index, const std::uint32_t ntiles) {
    ...
    std::uint32_t offset_address = MUL_TILE_SIZE_AND_INDEX((uint)unpack_src_format[input], 1);
    std::uint32_t address = base_address;
